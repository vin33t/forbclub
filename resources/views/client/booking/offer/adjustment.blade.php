...
@section('page-script')
  ...
  <script>
    function addMoreService(){
      var service =
        ...
        '<div class="col-md-4">'+
        '<label for="service_type">Service Type</label>'+
        '<select name="service_type[]" onchange="serviceMoreType(this);" class="form-control check-count" required>'+
        '<option value="">--Select Service Type--</option>'+
        '<option value="Hotel">Hotel</option>'+
        '<option value="Flight">Flight</option>'+
        '<option value="Visa">Visa</option>'+
  ...
  <script>
    function checkCount(){
      var services = document.getElementsByClassName('check-count');
      if(!services.length){
        $('.submit').html('<button type="button" id="submit" onclick="swalIt();" class="btn btn-md btn-info">Store Adjustment Offer</button>');
      }else{
        $('.submit').html('<button type="submit" id="submit" class="btn btn-md btn-info">Store Adjustment Offer</button>');
      }
